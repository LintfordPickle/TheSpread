...
public class ParticleBulletCollisionModifier implements IParticleModifier {
	...
	private ProjectileController mGameParticleController;

	// --------------------------------------
	// Constructor
	// --------------------------------------

	public ParticleBulletCollisionModifier(ProjectileController pParticleController, GameWorld pGameWorld) {
		...
		if (lRegion.type() == World.TILE_TYPE_SPAWNER) {
			if (lHealth[lTileIndex] >= 0) {
				...
				if (lSpreadDepthAct <= 1)
					lSpreadDepthAct = 1f;
				lHealth[lTileIndex] -= 50f / lSpreadDepthAct;
				if (lHealth[lTileIndex] <= 0) {
